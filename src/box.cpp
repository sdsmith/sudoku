...
void Cell::add_possibility(char c) {
...
bool Box::contains(char c) noexcept {
    ...
    for (s32 y = y_offset; y < y_offset + cell_per_box; ++y) {
        for (s32 x = x_offset; x < x_offset + cell_per_box; ++x) {
            if (grid->get(x, y).c == c) { return true; }
        }
    }
...
bool Box::row_contains(u8 row, char c) const noexcept {
    ...
    for (s32 x = x_offset; x < x_offset + cell_per_box; ++x) {
        if (grid->get(x, y_offset + row).c == c) { return true; }
    }
...
bool Box::col_contains(u8 col, char c) const noexcept
{
    ...
    for (s32 y = y_offset; y < y_offset + cell_per_box; ++y) {
        if (grid->get(x_offset + col, y).c == c) { return true; }
    }

    return false;
}

AdjacentBoxRow::AdjacentBoxRow(std::vector<Box>&& boxes) : m_boxes(std::move(boxes)) {}
...
AdjacentBoxCol::AdjacentBoxCol(std::vector<Box>&& boxes) : m_boxes(std::move(boxes)) {}
